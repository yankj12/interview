...
public class EmailConfigMongoDaoUtil {
	...
	public String insertEmailConfig(EmailConfig emailConfig){
		...
		//Access a Collection
		MongoCollection<Document> collection = database.getCollection("EmailConfig");
		
		//Create a Document
		Document doc = (Document)SchameDocumentUtil.schameToDocument(emailConfig, EmailConfig.class);;
	...
	public List<EmailConfig> findEmailConfigDocumentsByCondition(Map<String, Object> condition){
		List<EmailConfig> emailConfigs = null;
		...
		if(condition != null && condition.size() > 0) {
			...
			if(bsons != null && bsons.size() > 0){
				docs = collection.find(Filters.and(bsons)).limit(limit).skip(skip).sort(new Document("firstEmailConfigTime", -1)).into(new ArrayList<Document>());
			}else{
				docs = collection.find().limit(limit).skip(skip).sort(new Document("firstEmailConfigTime", -1)).into(new ArrayList<Document>());
			}
			
			if(docs != null){
				emailConfigs = new ArrayList<EmailConfig>();
				
				for(Document doc : docs){
					EmailConfig emailConfig = new EmailConfig();
					//将document转换为emailConfig
					//System.out.println(doc.get("_id"));
					emailConfig = (EmailConfig)SchameDocumentUtil.documentToSchame(doc, EmailConfig.class);
	...
	public Long countEmailConfigDocumentsByCondition(Map<String, Object> condition){
		long count = 0L;
	...
	public EmailConfig findEmailConfigById(String id) {
		...
		if(id!= null && !"".equals(id.trim())) {
			...
			if(docs != null && docs.size() > 0) {
				emailConfig = (EmailConfig)SchameDocumentUtil.documentToSchame(docs.get(0), EmailConfig.class);
			}
			mongoClient.close();
		}
	...
	public List<EmailConfig> findEmailConfigsByUserCode(String userCode){
		//To connect to a single MongoDB instance:
	    //You can explicitly specify the hostname and the port:
		MongoCredential credential = MongoCredential.createCredential(dataSource.getUser(), dataSource.getDbUserDefined(), dataSource.getPassword().toCharArray());
		MongoClient mongoClient = new MongoClient(new ServerAddress(dataSource.getIp(), dataSource.getPort()),
