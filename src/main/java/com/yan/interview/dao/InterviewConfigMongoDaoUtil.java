...
public class InterviewConfigMongoDaoUtil {
	...
	public String insertInterviewConfig(InterviewConfig interviewConfig){
		...
		//Access a Collection
		MongoCollection<Document> collection = database.getCollection("InterviewConfig");
		
		//Create a Document
		Document doc = (Document)SchameDocumentUtil.schameToDocument(interviewConfig, InterviewConfig.class);;
	...
	public List<InterviewConfig> findInterviewConfigDocumentsByCondition(Map<String, Object> condition){
		List<InterviewConfig> interviewConfigs = null;
		...
		if(condition != null && condition.size() > 0) {
				...
				if(value != null && !"".equals(value.toString().trim())){
					if("id".equals(key)) {
						bsons.add(Filters.eq("_id", new ObjectId(value.toString())));
							...
							|| "name".equals(key) 
							|| "interviewOfficerPhone".equals(key)) {
			...
			if(bsons != null && bsons.size() > 0){
				docs = collection.find(Filters.and(bsons)).limit(limit).skip(skip).sort(new Document("firstInterviewConfigTime", -1)).into(new ArrayList<Document>());
			}else{
				docs = collection.find().limit(limit).skip(skip).sort(new Document("firstInterviewConfigTime", -1)).into(new ArrayList<Document>());
			}
			
			if(docs != null){
				interviewConfigs = new ArrayList<InterviewConfig>();
				
				for(Document doc : docs){
					InterviewConfig interviewConfig = new InterviewConfig();
					//将document转换为interviewConfig
					//System.out.println(doc.get("_id"));
					interviewConfig = (InterviewConfig)SchameDocumentUtil.documentToSchame(doc, InterviewConfig.class);
					
					interviewConfigs.add(interviewConfig);
				}
			}
			mongoClient.close();
		}
	...
	public Long countInterviewConfigDocumentsByCondition(Map<String, Object> condition){
		long count = 0L;
	...
	public InterviewConfig findInterviewConfigById(String id) {
		...
		if(id!= null && !"".equals(id.trim())) {
			...
			if(docs != null && docs.size() > 0) {
				interviewConfig = (InterviewConfig)SchameDocumentUtil.documentToSchame(docs.get(0), InterviewConfig.class);
			}
			mongoClient.close();
		}
	...
	public List<InterviewConfig> findInterviewConfigsByUserCode(String userCode){
		//To connect to a single MongoDB instance:
	    //You can explicitly specify the hostname and the port:
		MongoCredential credential = MongoCredential.createCredential(dataSource.getUser(), dataSource.getDbUserDefined(), dataSource.getPassword().toCharArray());
		MongoClient mongoClient = new MongoClient(new ServerAddress(dataSource.getIp(), dataSource.getPort()),
		...
		List<Document> docs = collection.find(Filters.eq("userCode", userCode)).into(new ArrayList<Document>());
	...
	public void updateInterviewConfig(InterviewConfig interviewConfig){
		//To connect to a single MongoDB instance:
	    //You can explicitly specify the hostname and the port:
		MongoCredential credential = MongoCredential.createCredential(dataSource.getUser(), dataSource.getDbUserDefined(), dataSource.getPassword().toCharArray());
		MongoClient mongoClient = new MongoClient(new ServerAddress(dataSource.getIp(), dataSource.getPort()),
	...
	public void updateValidStatus(String id, String validStatus){
		//To connect to a single MongoDB instance:
	    //You can explicitly specify the hostname and the port:
		MongoCredential credential = MongoCredential.createCredential(dataSource.getUser(), dataSource.getDbUserDefined(), dataSource.getPassword().toCharArray());
		MongoClient mongoClient = new MongoClient(new ServerAddress(dataSource.getIp(), dataSource.getPort()),
